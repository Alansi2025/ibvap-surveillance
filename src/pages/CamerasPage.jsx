...
const CamerasPage = () => {
  const [camerasList, setCamerasList] = useState(initialCamerasData);
  ...
  // Toggle Camera Status (Turn Off / Turn On)
  const toggleCameraStatus = (id) => {
    setCamerasList((prev) =>
      prev.map((cam) => {
        if (cam.id === id) {
          const isNowOffline = cam.status !== 'offline';
          return {
            ...cam,
            status: isNowOffline ? 'offline' : 'online',
            statusText: isNowOffline ? 'Offline' : 'Online',
            isOffline: isNowOffline,
            lastSeen: isNowOffline ? 'Powered Off' : 'Just now',
            type: isNowOffline ? 'offline' : 'online',
          };
        }
        return cam;
      })
  ...
  // Master Power Toggle
  const activeCount = camerasList.filter((c) => c.status !== 'offline').length;
  ...
  // Filtered Cameras
  const filteredCameras = camerasList.filter((cam) => {
    const matchesSearch = 
    ...
    const matchesResolution = resolutionFilter === 'all' || cam.resolution === resolutionFilter;

    return matchesSearch && matchesStatus && matchesLocation && matchesResolution;
  });
  ...
  const warningCount = camerasList.filter((c) => c.status === 'warning').length;
  const offlineCount = camerasList.filter((c) => c.status === 'offline').length;
    ...
    <div className="cameras-page-container">
      ...
      <div className="page-header-row">
        ...
        <div className="header-status-pills font-mono">
          <button 
            className={`master-power-btn ${activeCount === 0 ? 'power-state-off' : 'power-state-on'}`}
            onClick={toggleAllCameras}
            title={activeCount === 0 ? "Turn On All Cameras" : "Turn Off All Cameras"}
          >
            <Power size={13} />
            <span>{activeCount === 0 ? "Turn On All Cameras" : "Turn Off All Cameras"}</span>
          </button>

          <span className={`pill-badge ${activeCount > 0 ? 'pill-green' : 'pill-red'}`}>
            <span className={`status-dot ${activeCount > 0 ? 'dot-green pulse-ring' : 'dot-red'}`}></span>
            {activeCount > 0 ? 'SYSTEM ONLINE' : 'ALL CAMERAS OFF'}
          </span>
      ...
      <div className="kpi-cards-grid">
        ...
        <div className="kpi-card">
          <div className="kpi-info font-mono">
            ...
            <span className="kpi-value">{camerasList.length}</span>
        ...
        <div className="kpi-card kpi-card-online">
          <div className="kpi-info font-mono">
            ...
            <span className="kpi-sub font-sans">
              Operational feeds ({((onlineCount / camerasList.length) * 100).toFixed(0)}%)
            </span>
          </div>
        </div>
        ...
        <div className="kpi-card">
          <div className="kpi-info font-mono">
            ...
            <span className="kpi-sub font-sans">Hardware unreachable / Powered off</span>
          </div>
        </div>
      </div>
      ...
      <div className="cameras-control-bar">
        ...
        <div className="control-right-group">
          <span className="showing-counter-text font-mono">
            Showing <strong>{filteredCameras.length}</strong> of <strong>{camerasList.length}</strong> cameras
          </span>
        ...
        {filteredCameras.map((cam) => (
          <div key={cam.id} className={`camera-manage-card card-type-${cam.type}`}>
            ...
            <div className={`cam-card-viewport scanlines ${cam.isOffline ? 'offline-viewport' : ''}`}>
              {!cam.isOffline ? (
                <>
                  <img src={cam.image} alt={cam.name} className="cam-viewport-img" />
                ...
                <div className="offline-error-content font-mono">
                  <div className="offline-top-row">
                    <span className="red-halted-text">● POWERED OFF</span>
                    <span className="no-rec-text">🚫 REC PAUSED</span>
                  </div>
                  ...
                  <div className="error-center-box">
                    ...
                    <div className="error-title-text">FEED DEACTIVATED: {cam.id}</div>
                    <div className="error-code-text">CAMERA STANDBY MODE</div>
                    <button 
                      className="btn-quick-turnon font-mono"
            ...
            <div className="cam-card-body">
              ...
              <div className="body-header-row">
                ...
                <div className="header-status-and-power">
                  {cam.status === 'online' && (
                  ...
                  <button 
                    className={`cam-power-toggle-btn font-mono ${cam.status === 'offline' ? 'is-off' : 'is-on'}`}
                    onClick={() => toggleCameraStatus(cam.id)}
                    title={cam.status === 'offline' ? "Turn On Camera" : "Turn Off Camera"}
                  >
                    <Power size={11} />
                    <span>{cam.status === 'offline' ? 'Turn On' : 'Turn Off'}</span>
                  </button>
                </div>
              </div>
              ...
              {/* Optional Detections / Last Detection Box */}
              {!cam.isOffline && cam.hasDetections && cam.detectionMode === 'c01_double' && (
              ...
              {!cam.isOffline && cam.hasDetections && cam.detectionMode === 'c02_patrol' && (
                <div className="detections-info-box font-mono">
                  <div className="box-header">
                    <span>CURRENT DETECTIONS</span>
              ...
              {!cam.isOffline && cam.hasDetections && cam.detectionMode === 'c03_alerts' && (
              ...
              {!cam.isOffline && cam.hasDetections && cam.detectionMode === 'c04_security' && (
                <div className="detections-info-box font-mono">
                  <div className="box-header">
                    <span>CURRENT DETECTIONS</span>
                    <span className="text-green">● 1 ACTIVE</span>
              ...
              {!cam.isOffline && cam.hasDetections && cam.detectionMode === 'c05_clear' && (
                <div className="detections-info-box font-mono">
                  <div className="box-header">
                    <span>CURRENT DETECTIONS</span>
              ...
              {(cam.isOffline || cam.detectionMode === 'c06_cached') && (
                <div className="detections-info-box font-mono box-offline-red">
                  <div className="box-header">
                    <span>FEED STATUS / ACTIVITY</span>
                    <span className="badge-no-feed font-mono">OFFLINE / STANDBY</span>
                  </div>
                  <div className="cached-activity-card font-mono">
                    <div className="cached-line1 font-mono">
                      <div className="det-left-info text-red">
                        <VideoOff size={13} className="text-red-alert" />
                        <span className="det-name-bold text-red-alert">Camera Power Off</span>
                      </div>
                      <span className="text-muted font-mono">Standby</span>
                    </div>
                    <div className="cached-line2 font-mono text-muted">
                      <span>Status: <strong>Sensor Inactive</strong></span>
                      <span className="text-muted font-bold">{cam.lastSeen}</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="cam-card-actions-group">
                <button 
                  className={`btn-cam-power-action font-mono ${cam.status === 'offline' ? 'btn-power-on-action' : 'btn-power-off-action'}`}
                  onClick={() => toggleCameraStatus(cam.id)}
                >
                  <Power size={13} />
                  <span>{cam.status === 'offline' ? 'Turn On Camera' : 'Turn Off Camera'}</span>
                </button>

                {!cam.isOffline ? (
                  <button className={`btn-cam-action ${cam.status === 'warning' ? 'btn-warning-glow' : ''}`}>
                    <Play size={13} fill="currentColor" />
                    <span>View Live</span>
                  </button>
                ) : (
                  <button className="btn-cam-action btn-offline">
                    <Wrench size={13} />
      ...
      <div className="cameras-table-footer font-mono">
        <div className="footer-count-text">
          Showing <strong>1–{filteredCameras.length}</strong> of <strong>{camerasList.length}</strong> registered cameras
        </div>
        ...
        <div className="pagination-controls">
          ...
          <button 
            className={`page-num-btn ${currentPage === 4 ? 'active' : ''}`}
